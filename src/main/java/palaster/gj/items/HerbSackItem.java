...
public class HerbSackItem extends SpecialModItem {
	...
	public void appendHoverText(ItemStack stack, World world, List<ITextComponent> tooltip, ITooltipFlag flag) {
		if(NBTHelper.getIntegerFromItemStack(stack, TAG_INT_SS) >= 0)
			tooltip.add(new StringTextComponent("Selected Spell: " + I18n.get("gj.job.botanist.spell." + NBTHelper.getIntegerFromItemStack(stack, TAG_INT_SS))));
	}
    ...
    public ActionResultType interactLivingEntity(ItemStack itemStack, PlayerEntity playerEntity, LivingEntity livingEntity, Hand hand) {
    	if(!playerEntity.level.isClientSide) {
	        ...
	        if(rpg != null && rpg.getJob() != null && rpg.getJob() instanceof JobBotanist) {
	            IBotanySpell IBS = BotanySpells.BOTANY_SPELLS.get(NBTHelper.getIntegerFromItemStack(itemStack, TAG_INT_SS));
    ...
    public ActionResult<ItemStack> use(World world, PlayerEntity playerEntity, Hand hand) {
    	if(!world.isClientSide) {
            if(playerEntity.isShiftKeyDown()) {
    			...
    			final IRPG rpg = lazy_optional_rpg.orElse(null);
                if(rpg != null && rpg.getJob() != null && rpg.getJob() instanceof JobBotanist) {
                    IBotanySpell IBS = BotanySpells.BOTANY_SPELLS.get(NBTHelper.getIntegerFromItemStack(playerEntity.getItemInHand(hand), TAG_INT_SS));
                    if(rpg.getMagick() < IBS.getCost())
    ...
    public ActionResultType useOn(ItemUseContext itemUseContext) {
    	if(!itemUseContext.getLevel().isClientSide) {
			...
			final IRPG rpg = lazy_optional_rpg.orElse(null);
            if(rpg != null && rpg.getJob() != null && rpg.getJob() instanceof JobBotanist) {
                IBotanySpell IBS = BotanySpells.BOTANY_SPELLS.get(NBTHelper.getIntegerFromItemStack(itemUseContext.getItemInHand(), TAG_INT_SS));
                if(rpg.getMagick() < IBS.getCost())
                    return ActionResultType.FAIL;
