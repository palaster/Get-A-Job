...
public class RPGIntroScreen extends ContainerScreen<RPGIntroContainer> {
	...
	protected void renderLabels(MatrixStack ms, int mouseX, int mouseY) {
		...
		if(rpg != null) {
            ...
            font.draw(ms, I18n.get("gj.expCost", RPGDefault.getExperienceCostForNextLevel(minecraft.player)), 6, 87, expColor);
            if(rpg.getJob() != null)
            	rpg.getJob().drawExtraInformationBase(ms, font, mouseX, mouseY, minecraft.player, 6, 99);
		}
	}
	...
	protected void init() {
		...
		addButton(new Button(leftPos + 124, topPos + 25, 16, 10, arrow, (button) -> PacketHandler.sendToServer(new ScreenButtonPacket(bHand, 0))));
		addButton(new Button(leftPos + 124, topPos + 37, 16, 10, arrow, (button) -> PacketHandler.sendToServer(new ScreenButtonPacket(bHand, 1))));
		addButton(new Button(leftPos + 124, topPos + 49, 16, 10, arrow, (button) -> PacketHandler.sendToServer(new ScreenButtonPacket(bHand, 2))));
		addButton(new Button(leftPos + 124, topPos + 61, 16, 10, arrow, (button) -> PacketHandler.sendToServer(new ScreenButtonPacket(bHand, 3))));
		addButton(new Button(leftPos + 124, topPos + 73, 16, 10, arrow, (button) -> PacketHandler.sendToServer(new ScreenButtonPacket(bHand, 4))));
		addButton(new Button(leftPos + 124, topPos + 85, 16, 10, arrow, (button) -> PacketHandler.sendToServer(new ScreenButtonPacket(bHand, 5))));
	}
}
